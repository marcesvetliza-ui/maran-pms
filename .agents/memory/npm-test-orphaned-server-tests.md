...
The same drift can occur in service tests that mock the Drizzle `db`: if production code starts using a method such as `db.update`, the mock must expose that method and its chained builder shape. A full test run is useful for catching this class of stale mock.
