...
export class DatabaseStorage implements IStorage {
  ...
  async getRoomTypes(): Promise<RoomType[]> {
    // This is the canonical catalog consumed by room-type selectors. It must
    // not depend on room inventory: a valid type can exist before its first
    // room is created or while all its rooms are inactive.
    return db.select().from(roomTypes).orderBy(
      sql`coalesce(${roomTypes.sortOrder}, 0)`,
      asc(roomTypes.name),
      asc(roomTypes.code),
    );
  }
  ...
  private async getRoomTypeReferenceRows(database: any = db) {
    ...
    return [
      { source: "rooms" as const, rows: roomRows },
      { source: "rate_plans" as const, rows: ratePlanRows },
      { source: "reservations" as const, rows: reservationRows },
      { source: "reservation_history" as const, rows: reservationHistoryRows },
      { source: "group_room_blocks" as const, rows: groupBlockRows },
      { source: "packages" as const, rows: packageRows },
      { source: "package_room_prices" as const, rows: packageRoomPriceRows },
    ];
  }
  ...
  async getOrphanedRoomTypeReferences(): Promise<OrphanedRoomTypeReference[]> {
    const [types, sources] = await Promise.all([
      db.select({ id: roomTypes.id }).from(roomTypes),
      this.getRoomTypeReferenceRows(),
    ]);
    const knownIds = new Set(types.map(type => type.id));
    const orphaned = new Map<string, RoomTypeReference[]>();

    for (const { source, rows } of sources) {
      const counts = new Map<string, number>();
      for (const row of rows) {
        if (!row.roomTypeId || knownIds.has(row.roomTypeId)) continue;
        counts.set(row.roomTypeId, (counts.get(row.roomTypeId) ?? 0) + 1);
      }
      for (const [roomTypeId, count] of counts) {
        const references = orphaned.get(roomTypeId) ?? [];
        references.push({ source, count });
        orphaned.set(roomTypeId, references);
      }
    }
    ...
    await db.transaction(async (tx) => {
      ...
      const replacements: Array<{
      ...
      }> = [
        {
          source: "rooms",
          count: 0,
          update: async () => tx.update(rooms).set({ roomTypeId: toRoomTypeId }).where(eq(rooms.roomTypeId, fromRoomTypeId)).returning({ id: rooms.id }),
        },
        {
          source: "rate_plans",
          count: 0,
          update: async () => tx.update(ratePlans).set({ roomTypeId: toRoomTypeId }).where(eq(ratePlans.roomTypeId, fromRoomTypeId)).returning({ id: ratePlans.id }),
        },
        {
          source: "reservations",
          count: 0,
          update: async () => tx.update(reservations).set({ roomTypeId: toRoomTypeId }).where(eq(reservations.roomTypeId, fromRoomTypeId)).returning({ id: reservations.id }),
        },
        {
          source: "reservation_history",
          count: 0,
          update: async () => tx.update(reservations).set({ originalRoomTypeId: toRoomTypeId }).where(eq(reservations.originalRoomTypeId, fromRoomTypeId)).returning({ id: reservations.id }),
        },
        {
          source: "group_room_blocks",
          count: 0,
          update: async () => tx.update(groupRoomBlocks).set({ roomTypeId: toRoomTypeId }).where(eq(groupRoomBlocks.roomTypeId, fromRoomTypeId)).returning({ id: groupRoomBlocks.id }),
        },
        {
          source: "packages",
          count: 0,
          update: async () => tx.update(packages).set({ roomTypeId: toRoomTypeId }).where(eq(packages.roomTypeId, fromRoomTypeId)).returning({ id: packages.id }),
        },
        {
          source: "package_room_prices",
          count: 0,
          update: async () => tx.update(packageRoomPrices).set({ roomTypeId: toRoomTypeId }).where(eq(packageRoomPrices.roomTypeId, fromRoomTypeId)).returning({ id: packageRoomPrices.id }),
        },
      ];

      for (const replacement of replacements) {
        const rows = await replacement.update();
        replacement.count = rows.length;
   ...
   * cash movements: the payment advisory lock is the cross-endpoint
   * idempotency gate.
   */
  async voidReservationPaymentAtomic(paymentId: string, operator: string, reason: string): Promise<{ alreadyVoided: boolean; paymentId: string }> {
    return db.transaction(async (tx) => {
      // Shift lock comes first, matching closeShift/payment creation. The
      // payment/event are re-read only after the owning shift is stable.
      await tx.execute(sql`
        SELECT cs.id FROM cash_shifts cs
        JOIN cash_movements cm ON cm.shift_id = cs.id
        WHERE cm.payment_id = ${paymentId}
        ORDER BY cs.id FOR UPDATE OF cs
      `);
      await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext(${`reservation-payment-void:${paymentId}`}))`);
      const paymentResult = await tx.execute(sql`SELECT * FROM payments WHERE id = ${paymentId} FOR UPDATE`);
      const payment = paymentResult.rows[0] as any;
      if (!payment) throw Object.assign(new Error("Pago no encontrado"), { statusCode: 404 });
      const classification = classifyReservationPaymentMethod(payment.method);
      const linked = await tx.execute(sql`
        SELECT * FROM cash_movements WHERE payment_id = ${paymentId} FOR UPDATE
      `);
      if (payment.status === "anulado") {
        return { alreadyVoided: true, paymentId };
      }
      const informational = classification.informational;
      if (informational) {
        const activeInfo = (linked.rows as any[]).filter((m) => m.movement_type === "informational" && !m.anulado);
        if (!activeInfo.length) {
          throw Object.assign(new Error("La liquidación no monetaria no tiene movimiento informativo activo"), { statusCode: 409 });
        }
        if (classification.method === "current_account") {
          const durableReferences = new Set<string>();
          if (payment.reference) durableReferences.add(String(payment.reference));
          if (payment.invoice_ref) {
            durableReferences.add(String(payment.invoice_ref));
            try {
              const invoice = JSON.parse(payment.invoice_ref);
              if (invoice.reference) durableReferences.add(String(invoice.reference));
              if (invoice.tipoComprobante && invoice.puntoVenta != null && invoice.numero != null) {
                durableReferences.add(`${invoice.tipoComprobante}-${String(invoice.puntoVenta).padStart(4, "0")}-${String(invoice.numero).padStart(8, "0")}`);
              }
            } catch {
              throw Object.assign(new Error("La referencia de factura del pago CC es inválida"), { statusCode: 409 });
            }
          }
          if (!durableReferences.size) {
            throw Object.assign(new Error("El pago CC no tiene referencia documental durable"), { statusCode: 409 });
          }
          const refs = Array.from(durableReferences);
          const cargo = await tx.execute(sql`
            SELECT * FROM account_movements
            WHERE reservation_id = ${payment.reservation_id} AND type = 'cargo'
              AND amount::numeric = ${payment.amount}::numeric
              AND reference IN (${sql.join(refs.map((reference) => sql`${reference}`), sql`, `)})
            FOR UPDATE
          `);
          if (cargo.rows.length !== 1) {
            throw Object.assign(new Error("El cargo CC exacto no existe o no es unívoco"), { statusCode: 409 });
          }
          const c = cargo.rows[0] as any;
          ...
          const reversal = await tx.execute(sql`
            ...
            WHERE entity_type = ${c.entity_type} AND entity_id = ${c.entity_id}
              AND reference = ${voidReference} FOR UPDATE
          `);
          if (!reversal.rows.length) {
            await tx.execute(sql`
              INSERT INTO account_movements
                (entity_type, entity_id, date, type, description, amount, reservation_id,
                 reference, payment_method, created_by)
              VALUES (${c.entity_type}, ${c.entity_id}, CURRENT_DATE, 'ajuste',
                ${`Anulación cargo CC — ${reason}`}, (-${c.amount}::numeric),
                ${payment.reservation_id}, ${voidReference}, 'current_account', ${operator})
            `);
          }
        }
        await tx.execute(sql`
          UPDATE cash_movements
          SET anulado = true, motivo_anulacion = ${reason}, anulado_por = ${operator}, anulado_at = NOW()
          WHERE payment_id = ${paymentId} AND movement_type = 'informational' AND anulado = false
        `);
        for (const event of activeInfo) {
          const shift = await tx.execute(sql`SELECT status FROM cash_shifts WHERE id = ${event.shift_id} FOR UPDATE`);
          if ((shift.rows[0] as any)?.status === "closed") {
            await tx.execute(sql`
              UPDATE cash_closing_summaries SET
                total_current_account = GREATEST(0, COALESCE(total_current_account, 0) - CASE WHEN ${classification.method} = 'current_account' THEN ${event.amount}::numeric ELSE 0 END),
                total_voucher = GREATEST(0, COALESCE(total_voucher, 0) - CASE WHEN ${classification.method} = 'voucher' THEN ${event.amount}::numeric ELSE 0 END),
                non_cash_settlements_total = GREATEST(0, COALESCE(non_cash_settlements_total, 0) - ${event.amount}::numeric),
                non_cash_settlements_count = GREATEST(0, COALESCE(non_cash_settlements_count, 0) - 1)
              WHERE shift_id = ${event.shift_id}
            `);
          }
        }
      } else {
        // Existing physical-cash behavior: preserve a linked expense reversal.
        const activeIncome = (linked.rows as any[]).find((m) => m.movement_type === "income" && !m.anulado);
        if (activeIncome) {
          await tx.insert(cashMovements).values({
            id: randomUUID(), shiftId: activeIncome.shift_id, area: activeIncome.area,
            sourceType: "payment_void", sourceId: paymentId,
            sourceLabel: `Anulación pago ${payment.method}`, paymentMethod: classification.method,
            amount: payment.amount, movementType: "expense", registeredBy: operator,
            paymentId,
          } as any);
        }
      }
      const folio = await tx.execute(sql`
        SELECT id FROM folios WHERE entity_type = 'reservation' AND entity_id = ${payment.reservation_id}
        LIMIT 1 FOR UPDATE
      `);
      if (folio.rows[0]) {
        const originalFolioPayment = await tx.execute(sql`
          SELECT id FROM folio_movements
          WHERE folio_id = ${(folio.rows[0] as any).id}
            AND type = 'payment'
            AND source_type = 'payment'
            AND source_id = ${paymentId}
          LIMIT 2 FOR UPDATE
        `);
        if (originalFolioPayment.rows.length !== 1) {
          throw Object.assign(new Error("El movimiento de pago original del folio no existe o no es unívoco"), { statusCode: 409 });
        }
        const originalFolioMovementId = (originalFolioPayment.rows[0] as any).id;
        const existingVoid = await tx.execute(sql`
          SELECT id FROM folio_movements
          WHERE folio_id = ${(folio.rows[0] as any).id} AND type = 'void'
            AND voided_movement_id = ${originalFolioMovementId} FOR UPDATE
        `);
        if (!existingVoid.rows.length) {
          await tx.execute(sql`
            INSERT INTO folio_movements
              (folio_id, type, amount, description, voided_movement_id, void_reason, registered_by)
            VALUES (${(folio.rows[0] as any).id}, 'void', ${(-Number(payment.amount)).toFixed(2)},
              ${`Anulación pago — ${reason}`}, ${originalFolioMovementId}, ${reason}, ${operator})
          `);
        }
        await tx.execute(sql`
          UPDATE folios f SET
            total_charges = COALESCE((SELECT SUM(amount::numeric) FROM folio_movements
              WHERE folio_id = f.id AND type IN ('charge','transfer_in')), 0),
            total_payments = COALESCE((SELECT SUM(amount::numeric) FROM folio_movements
              WHERE folio_id = f.id AND type IN ('payment','advance','discount','transfer_out','void')), 0)
          WHERE f.id = ${(folio.rows[0] as any).id}
        `);
        await tx.execute(sql`
          UPDATE folios SET balance = total_charges::numeric - total_payments::numeric
          WHERE id = ${(folio.rows[0] as any).id}
        `);
      }
      await tx.execute(sql`
        UPDATE payments SET status = 'anulado', anulado_por = ${operator},
          motivo_anulacion = ${reason}, anulado_at = NOW()
        WHERE id = ${paymentId} AND status IS DISTINCT FROM 'anulado'
      `);
      return { alreadyVoided: false, paymentId };
    });
  }
  ...
  async createReservationPaymentWithLedger(input: {
    ...
    const methodMap: Record<string, string> = {
      ...
      gift_voucher_room: "voucher",
      voucher_regalo: "voucher",
      room_charge_voucher: "voucher",
    ...
    return db.transaction(async (tx) => {
      ...
      const ensureInformationalMovement = async (
        paymentId: string,
        method: string,
        amount: string,
        ...
        if (!informationalMethods.has(method)) return;
        const existing = await tx.execute(sql`
          SELECT id FROM cash_movements
          WHERE payment_id = ${paymentId} AND movement_type = 'informational'
          LIMIT 1
        `);
        ...
        if (!shiftId) {
          throw Object.assign(
            new Error("No hay un turno de recepción abierto para registrar la liquidación no monetaria."),
            { statusCode: 409 },
          );
        }
        await tx.execute(sql`
          SELECT pg_advisory_xact_lock(hashtext(${`reservation-informational-movement:${paymentId}`}))
        `);
        const afterLock = await tx.execute(sql`
          SELECT id FROM cash_movements
          WHERE payment_id = ${paymentId} AND movement_type = 'informational'
          LIMIT 1
        `);
        if (afterLock.rows.length) return;
        const stillOpen = await tx.execute(sql`
          SELECT id FROM cash_shifts WHERE id = ${shiftId} AND status = 'open' FOR UPDATE
        `);
        if (!stillOpen.rows.length) {
          throw Object.assign(new Error("El turno de recepción se cerró durante la liquidación."), { statusCode: 409 });
        }
        await tx.insert(cashMovements).values({
          id: randomUUID(),
          shiftId,
          ...
          amount,
          movementType: "informational",
          registeredBy: operator || null,
          paymentId,
        } as any);
      };
      // Invoice references are the durable idempotency key for a settlement.
      // Recovery after an ARCA/local failure must not create another folio
      // payment or another CC cargo.
      if (input.accountSettlement && input.payment.invoiceRef) {
        await tx.execute(sql`
          SELECT pg_advisory_xact_lock(hashtext(${`reservation-cc-settlement:${input.payment.reservationId}:${String(input.payment.invoiceRef)}`}))
        `);
        const [existing] = await tx.select().from(payments).where(and(
          eq(payments.reservationId, input.payment.reservationId),
          eq(payments.method, "cuenta_corriente"),
          eq(payments.invoiceRef, String(input.payment.invoiceRef)),
          or(isNull(payments.status), eq(payments.status, "active")),
        )).limit(1);
        if (existing) {
          await ensureInformationalMovement(
            existing.id,
            "current_account",
          ...
          if (input.accountSettlement.invoiceId) {
            await tx.execute(sql`
              ...
              SET payment_id = ${existing.id}
              WHERE id = ${input.accountSettlement.invoiceId}
                AND (payment_id IS NULL OR payment_id = ${existing.id})
            `);
          }
          return existing;
        }
      ...
      if (input.accountSettlement) {
        const existingCargo = input.accountSettlement.existingCargoId
          ? await tx.execute(sql`
              SELECT id, reservation_id, reference, description FROM account_movements
              WHERE id = ${input.accountSettlement.existingCargoId}
                AND entity_type = ${input.accountSettlement.entityType}
                AND entity_id = ${input.accountSettlement.entityId}
                AND type = 'cargo'
                AND amount::numeric = ${canonicalAmount}
              LIMIT 1 FOR UPDATE
            `)
          : input.accountSettlement.reference
          ? await tx.execute(sql`
              SELECT id FROM account_movements
              WHERE entity_type = ${input.accountSettlement.entityType}
                AND entity_id = ${input.accountSettlement.entityId}
                AND reference = ${input.accountSettlement.reference}
                AND type = 'cargo'
              LIMIT 1 FOR UPDATE
            `)
          : { rows: [] };
        if (input.accountSettlement.existingCargoId && !existingCargo.rows.length) {
          throw Object.assign(new Error("El cargo CC histórico ya no coincide con la liquidación"), { statusCode: 409 });
        }
        if (input.accountSettlement.existingCargoId) {
          const cargo = existingCargo.rows[0] as any;
          if (cargo.reservation_id && cargo.reservation_id !== payment.reservationId) {
            throw Object.assign(new Error("El cargo CC histórico ya pertenece a otra reserva"), { statusCode: 409 });
          }
          const canonical = input.accountSettlement.adoptedCanonicalReference;
          if (canonical && cargo.reference !== input.accountSettlement.reference && cargo.reference !== canonical) {
            throw Object.assign(new Error("El cargo CC histórico referencia otro comprobante"), { statusCode: 409 });
          }
          if (canonical) {
            await tx.execute(sql`
              UPDATE account_movements
              SET reservation_id = ${payment.reservationId},
                  reference = ${canonical},
                  description = CASE
                    WHEN reference = ${canonical} THEN description
                    ELSE description || ' [referencia histórica: ' || COALESCE(reference, '') || ']'
                  END
              WHERE id = ${input.accountSettlement.existingCargoId}
            `);
          }
        }
        if (!existingCargo.rows.length) await tx.insert(accountMovements).values({
          entityType: input.accountSettlement.entityType,
          entityId: input.accountSettlement.entityId,
          date: input.payment.date,
          type: "cargo",
          description: input.accountSettlement.description,
          amount: canonicalAmount,
        ...
        if (input.accountSettlement.invoiceId) {
          for (const advanceId of input.accountSettlement.advancePaymentIds || []) {
            const lockedAdvance = await tx.execute(sql`
              ...
              WHERE id = ${advanceId}
                AND reservation_id = ${payment.reservationId}
                AND (status IS NULL OR status = 'active')
                AND method NOT IN ('cuenta_corriente','current_account')
              FOR UPDATE
            `);
            const advance = lockedAdvance.rows[0] as any;
            let existingInvoiceId: number | null = null;
            if (advance?.invoice_ref) {
              try { existingInvoiceId = Number(JSON.parse(advance.invoice_ref).id); } catch { existingInvoiceId = NaN; }
            }
            if (!advance || (advance.invoice_ref && existingInvoiceId !== input.accountSettlement.invoiceId)) {
              throw Object.assign(new Error("Un anticipo histórico ya no coincide con la factura"), { statusCode: 409 });
            }
            await tx.execute(sql`
              UPDATE payments
              SET invoice_link_failed = false,
                  invoice_ref = COALESCE(invoice_ref, ${input.accountSettlement.advanceInvoiceRef ?? null})
              WHERE id = ${advanceId}
            `);
          }
          await tx.execute(sql`
      ...
      for (const block of groupBlocks) {
        ...
        if (assignedCount < block.quantity) {
          ...
          unassignedGroupBlocks.push({
            ...
            roomTypeName: rt?.name || `Tipo no encontrado (${block.roomTypeId})`,
  ...
  async closeShift(
    ...
    return db.transaction(async (tx) => {
    const [shift] = await tx.select().from(cashShifts).where(eq(cashShifts.id, shiftId)).for("update");
    ...
    if (turnoTipo && !shift.turnoTipo) {
      await tx.update(cashShifts).set({ turnoTipo }).where(eq(cashShifts.id, shiftId));
      shift.turnoTipo = turnoTipo;
    }

    const movements = await tx.select().from(cashMovements).where(
    ...
    const methodNorm: Record<string, string> = {
      ...
      voucher: "voucher", gift_voucher: "voucher", voucher_regalo: "voucher",
      room_charge_voucher: "voucher", voucher_habitacion: "voucher",
    };
    const totals: Record<string, number> = {
      cash: 0, debit_card: 0, credit_card: 0, transfer: 0,
      mercadopago: 0, current_account: 0, room_charge: 0, voucher: 0,
    };
    let nonCashSettlementsTotal = 0;
    let nonCashSettlementsCount = 0;
    let totalGeneral = 0;
    ...
    const [summary] = await tx.insert(cashClosingSummaries).values({
      id: randomUUID(),
      shiftId,
      area: shift.area,
      ...
      totalVoucher: totals.voucher.toFixed(2),
      nonCashSettlementsTotal: nonCashSettlementsTotal.toFixed(2),
      nonCashSettlementsCount,
    ...
    const [updatedShift] = await tx.update(cashShifts)
    ...
    const [turnoNuevo] = await tx.insert(cashShifts).values({
      id: randomUUID(),
      area: shift.area,
      shiftNumber: nextNum,
