...
export function buildArcaQrUrl(factura: any, config: any): string | null {
  const cae = String(factura.cae ?? "");
  const tipoKey = String(factura.tipo_comprobante ?? factura.tipoComprobante ?? "");
  const tipoCmp = Number(TIPO_LABELS[tipoKey]?.codigo);
  const cuit = Number(String(config?.arcaCuit ?? config?.cuit ?? "").replace(/\D/g, ""));
  const fechaValue = factura.fecha_emision ?? factura.fechaEmision;
  const fecha = fechaValue instanceof Date
    ? fechaValue.toISOString().slice(0, 10)
    : String(fechaValue ?? "").slice(0, 10);
  if (!cae || !tipoCmp || !cuit || !fecha) return null;
  const recipient = getInvoiceRecipientDocument(factura);
  const payload = {
    ver: 1,
    fecha,
    cuit,
    ptoVta: Number(factura.punto_venta ?? factura.puntoVenta ?? 0),
    tipoCmp,
    nroCmp: Number(factura.numero ?? 0),
    importe: $n(factura.monto_total ?? factura.montoTotal ?? factura.total),
    moneda: "PES",
    ctz: 1,
    ...recipient,
    tipoCodAut: "E",
  ...
  return `https://www.afip.gob.ar/fe/qr/?p=${encodeURIComponent(Buffer.from(JSON.stringify(payload), "utf8").toString("base64"))}`;
}
...
export async function generarFacturaPDF(
  ...
  const qrUrl = buildArcaQrUrl(factura, config);
  const qrBuffer = qrUrl
    ? await QRCode.toBuffer(qrUrl, { type: "png", width: 180, margin: 1, errorCorrectionLevel: "M" })
    : null;
  return new Promise((resolve, reject) => {
    ...
    if (discriminaIVA && montoNeto !== 0)      totRows.push(["Importe Neto Gravado", montoNeto]);
    if (discriminaIVA && montoExento !== 0)    totRows.push(["Importe Exento",       montoExento]);
    ...
    if (transparenciaFiscal) {
      const ivaContenido = montoIva21 + montoIva105;
      box(x0, y, W, 30, "#9ca3af");
      doc.font("Helvetica-Bold").fontSize(7.5).fillColor("#111827")
        .text("Régimen de Transparencia Fiscal al Consumidor (Ley 27.743)", x0 + 6, y + 5, { width: W - 12 });
      doc.font("Helvetica").fontSize(7.5)
    ...
    const caeH = 64;
    box(x0, y, W, caeH);
    doc.moveTo(x0 + W * 0.66, y).lineTo(x0 + W * 0.66, y + caeH).strokeColor("#ccc").lineWidth(0.5).stroke();

    if (qrBuffer) doc.image(qrBuffer, x0 + 5, y + 5, { fit: [54, 54] });
    doc.font("Helvetica-Bold").fontSize(7.5).fillColor("#333")
       .text("Observaciones ARCA: Comprobante Autorizado", x0 + 65, y + 8, { width: W * 0.52 })
       .fillColor("#000").font("Helvetica").fontSize(8)
       .text(`CAE Nº: ${cae ?? "—"}`, x0 + 65, y + 25, { width: W * 0.52 })
       .text(`Fecha Venc. CAE: ${fDate(caeFechaVto)}`, x0 + 65, y + 41, { width: W * 0.52 });

    doc.font("Helvetica-Bold").fontSize(7.5)
       .text("Condición de Venta:", x0 + W * 0.68, y + 15, { width: W * 0.3 })
       .font("Helvetica").fontSize(8)
       .text(condVenta, x0 + W * 0.68, y + 31, { width: W * 0.3 });
