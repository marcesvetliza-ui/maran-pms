...
export function registerReservationsRoutes(app: Express) {
  ...
  app.post("/api/reservations/:id/check-out", async (req, res) => {
    ...
    try {
      ...
      if (forceCheckout && balance > 0.01) {
        ...
        const legacyInvoiceCargo = await db.execute(sql`
          SELECT 1
          FROM account_movements m
          JOIN sales_invoices si
            ON m.reference = si.tipo_comprobante || '-' || lpad(si.numero::text, 8, '0')
          WHERE si.reserva_id = ${reservation.id}
            AND (m.reservation_id = ${reservation.id} OR m.reservation_id IS NULL)
            AND m.type = 'cargo'
            AND si.estado IN ('emitida','parcial')
            AND si.cash_forma_pago = 'cuenta_corriente'
          LIMIT 1
        `);
        const alreadyHasDebtCargo = legacyInvoiceCargo.rows.length > 0 || existingMov.some(
    ...
    try {
      ...
      if (req.body.method === "cuenta_corriente" && reservationForPayment) {
        ...
        if (entityId) {
          ...
          try {
            const parsed = JSON.parse(String(req.body.invoiceRef || ""));
            if (Number.isInteger(Number(parsed?.id))) invoiceId = Number(parsed.id);
          } catch { /* legacy/manual payments have no invoice identity */ }
          accountSettlement = {
            entityType: billingTarget as "company" | "agency" | "guest",
            entityId,
            description: `Estadía ${reservationForPayment.reservationCode} — Hab. ${reservationForPayment.room?.roomNumber || reservationForPayment.roomId}`,
            reference: req.body.invoiceRef || undefined,
  ...
  app.patch("/api/payments/:id/anular", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const paymentClassification = classifyReservationPaymentMethod(pay.method);
      const normalizedPaymentMethod = paymentClassification.method;
      const isNonCashSettlement = paymentClassification.informational;
      const atomicVoid = await storage.voidReservationPaymentAtomic(req.params.id, operator, motivoAnulacion);
      return res.json({ ...atomicVoid, payment: { id: pay.id, status: "anulado" } });
      ...
      if (pay.reservation_id) {
        ...
        try {
          if (isNonCashSettlement) {
            await db.execute(sql`
              UPDATE cash_movements
              SET anulado = true, motivo_anulacion = ${motivoAnulacion},
                  anulado_por = ${operator}, anulado_at = NOW()
              WHERE payment_id = ${pay.id} AND movement_type = 'informational'
                AND anulado = false
            `);
            // Current-account cargoes are reversed in the account ledger, never
            // as a Caja expense. The void payment id makes retries harmless.
            if (normalizedPaymentMethod === "cuenta_corriente" || normalizedPaymentMethod === "current_account") {
              await db.execute(sql`
                ...
                SELECT entity_type, entity_id, CURRENT_DATE, 'ajuste',
                  ...
                  reservation_id, ${`void:${pay.id}`}, 'current_account', ${operator}
                FROM account_movements cargo
                WHERE cargo.reservation_id = ${pay.reservation_id}
                  AND cargo.type = 'cargo'
                  AND cargo.amount::numeric = ${pay.amount}::numeric
                    AND cargo.reference IS NOT DISTINCT FROM ${pay.reference || null}
                  AND NOT EXISTS (
                    SELECT 1 FROM account_movements a
                    WHERE a.reference = ${`void:${pay.id}`}
                  )
