    ...
    try {
      const result = await db.execute(sql`
        ...
        WHERE (p.status IS NULL OR p.status = 'active')
          AND p.group_payment_id IS NULL
          AND lower(replace(replace(COALESCE(p.method, ''), ' ', '_'), '-', '_')) IN (
            'efectivo', 'cash', 'tarjeta_debito', 'debit_card',
            'tarjeta_credito', 'credit_card', 'transferencia', 'transfer',
            'mercadopago', 'cuenta_corriente', 'current_account',
            'voucher', 'gift_voucher', 'gift_voucher_room', 'voucher_regalo', 'room_charge_voucher',
            'voucher_habitacion'
          )
          AND NOT EXISTS (
            SELECT 1 FROM cash_movements cm
            WHERE cm.payment_id = p.id AND cm.anulado = false
              AND (
                (
                  lower(replace(replace(COALESCE(p.method, ''), ' ', '_'), '-', '_')) IN
                    ('cuenta_corriente', 'current_account', 'voucher', 'gift_voucher',
                     'gift_voucher_room', 'voucher_regalo', 'room_charge_voucher', 'voucher_habitacion')
                  AND cm.movement_type = 'informational'
                )
                OR (
                  lower(replace(replace(COALESCE(p.method, ''), ' ', '_'), '-', '_')) NOT IN
                    ('cuenta_corriente', 'current_account', 'voucher', 'gift_voucher',
                     'gift_voucher_room', 'voucher_regalo', 'room_charge_voucher', 'voucher_habitacion')
                  AND cm.movement_type IN ('income', 'expense')
                )
              )
          )
  ...
  app.post("/api/cash/reservation-payments/:paymentId/repair-movement", requireRole(["admin", "manager"]), async (req, res) => {
    ...
    try {
      const outcome = await db.transaction(async (tx) => {
        ...
        const paymentMethod = classifyReservationPaymentMethod(payment.method).method;
        if (!["cash", "debit_card", "credit_card", "transfer", "mercadopago", "current_account", "voucher"].includes(paymentMethod)) {
          throw Object.assign(new Error("Este método no representa una liquidación recuperable de Caja."), { statusCode: 409 });
        }
        const isInformational = classifyReservationPaymentMethod(payment.method).informational;

        // Recheck after locking. A unique index is not assumed on legacy data,
        // so the locked payment row is the idempotency gate.
        const duplicate = await tx.execute(isInformational ? sql`
          SELECT id FROM cash_movements
          WHERE payment_id = ${paymentId} AND movement_type = 'informational' AND anulado = false
          LIMIT 1 FOR UPDATE
        ` : sql`
          SELECT id FROM cash_movements
          WHERE payment_id = ${paymentId} AND anulado = false
          LIMIT 1 FOR UPDATE
        `);
        ...
        await tx.insert(cashMovements).values({
          ...
          movementType: isInformational ? "informational" : "income",
          registeredBy: (req.user as any)?.fullName || (req.user as any)?.username || "Sistema",
          receiptType: "recuperacion_historica",
          paymentId,
        } as any);
        // Closed shifts have a materialized summary. Informational settlements
        // are persisted for reconciliation but never increase cash-bearing
        // income, totalGeneral, expected cash, or difference.
        if (isInformational) await tx.execute(sql`
          UPDATE cash_closing_summaries
          SET
            total_current_account = COALESCE(total_current_account, 0) + CASE WHEN ${paymentMethod} = 'current_account' THEN ${payment.amount}::numeric ELSE 0 END,
            total_voucher = COALESCE(total_voucher, 0) + CASE WHEN ${paymentMethod} = 'voucher' THEN ${payment.amount}::numeric ELSE 0 END,
            non_cash_settlements_total = COALESCE(non_cash_settlements_total, 0) + ${payment.amount}::numeric,
            non_cash_settlements_count = COALESCE(non_cash_settlements_count, 0) + 1
          WHERE shift_id = ${shift.id}
        `);
        else await tx.execute(sql`
          UPDATE cash_closing_summaries
          SET
            total_cash = COALESCE(total_cash, 0) + CASE WHEN ${paymentMethod} = 'cash' THEN ${payment.amount}::numeric ELSE 0 END,
  ...
  app.patch("/api/cash/movements/:id/anular", requireAuth, async (req, res) => {
    try {
      ...
      if (mov.paymentId && mov.sourceType === "reservation") {
        const atomicVoid = await storage.voidReservationPaymentAtomic(mov.paymentId, operator, motivoAnulacion);
        return res.json({ ...atomicVoid, movementId: mov.id });
      }
      ...
      if (mov.paymentId) {
        try {
          ...
          if (pay && pay.status !== "anulado") {
             const paymentClass = classifyReservationPaymentMethod(pay.method);
            ...
            `);
             if (paymentClass.informational && paymentClass.method === "current_account") {
               await db.execute(sql`
                 INSERT INTO account_movements
                   (entity_type, entity_id, date, type, description, amount,
                    reservation_id, reference, payment_method, created_by)
                 SELECT entity_type, entity_id, CURRENT_DATE, 'ajuste',
                   ${`Anulación cargo CC — ${motivoAnulacion}`}, -amount,
                   reservation_id, ${`void:${mov.paymentId}`}, 'current_account', ${operator}
                 FROM account_movements cargo
                 WHERE cargo.reservation_id = ${pay.reservation_id}
                   AND cargo.type = 'cargo' AND cargo.amount::numeric = ${pay.amount}::numeric
                   AND cargo.reference IS NOT DISTINCT FROM ${pay.reference || null}
                   AND NOT EXISTS (
                     SELECT 1 FROM account_movements a WHERE a.reference = ${`void:${mov.paymentId}`}
                   )
                 ORDER BY cargo.created_at DESC LIMIT 1
               `);
             }
