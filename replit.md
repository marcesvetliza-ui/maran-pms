...
- **Motor de Reservas Online (Booking Engine)**: Public 4-step booking wizard at `/reservar` (no login required, embeddable via iframe). Step 1: date/pax selector. Step 2: room type cards with photos, amenities, price. Step 3: guest data form. Step 4: confirmation with reservation code. Backend public API: `GET /api/public/booking/availability` (real-time availability check), `GET /api/public/booking/hotel-info`, `POST /api/public/booking/confirm` (creates guest+reservation in PMS). Admin configuration page at `/admin/booking-engine` to set photos, amenities, publicDescription, sort order, and visibility per room type. Fields added to `room_types` table: `public_description`, `amenities` (text[]), `photos` (text[]), `sort_order`, `show_in_booking`. Page is iframe-embeddable from any origin. Auth bypass fixed for `/api/public/` prefix.
